const BASE_URL = "https://m.douyu.com/"
...
const secParse = _ => {
    ...
    params.rid = parseInt(MY_URL.replace("https://m.douyu.com/", ""))
    // params.v = "2501"+getFormatDate()
    params.tt = Date.parse(new Date()).toString().substr(0,10)
    params.did = did[0].replace("\"did\":\"", "").replace("\"", "")
    ...
    const stream_json = fetch('https://m.douyu.com/api/room/ratestream', {headers:{'content-type':'application/json'}, body: param_body, method:'POST'})
    setError(stream_json)
